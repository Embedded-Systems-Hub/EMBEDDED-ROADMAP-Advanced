...
static int flash_init_wrapper(void *dev)
{
    ...
    return hal_spi_init(d->spi_ctx);
}

static int flash_read_wrapper(void *dev, uint32_t addr, uint8_t *data, size_t len)
{
    HAL_Flash_Device_t *d = dev;
    ...
    hal_spi_cs_low(d->spi_ctx, d->cs_pin);
    hal_spi_write(d->spi_ctx, cmd, sizeof(cmd));
    hal_spi_read(d->spi_ctx, data, len);
    hal_spi_cs_high(d->spi_ctx, d->cs_pin);

    return (int)len;
}

static int flash_write_wrapper(void *dev, uint32_t addr, const uint8_t *data, size_t len)
{
    HAL_Flash_Device_t *d = dev;
    ...
    hal_spi_cs_low(d->spi_ctx, d->cs_pin);
    hal_spi_write(d->spi_ctx, cmd, sizeof(cmd));
    hal_spi_write(d->spi_ctx, data, len);
    hal_spi_cs_high(d->spi_ctx, d->cs_pin);

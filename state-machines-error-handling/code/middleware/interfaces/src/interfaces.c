...
/*
 * Populated by the platform-specific configuration file.
 * This function registers all available interface contexts.
 */
void interfaces_cfg_init(void);

/*
 * Fixed-size registration table (sufficient for this system).
 * Each entry associates a string name with a hardware context pointer.
 */
typedef struct
{
    const char *name;

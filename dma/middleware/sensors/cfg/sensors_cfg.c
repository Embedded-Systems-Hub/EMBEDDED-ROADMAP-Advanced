...
typedef struct { const void *adc_ctx; uint8_t channel; void *data; } Potentiometer_Device_t;

static Potentiometer_Device_t pot_dev;

static status_t potentiometer_init_wrapper(void)
{
	status_t ret  = interfaces_get("adc_pot", &pot_dev.adc_ctx, &pot_dev.data);
    pot_dev.channel  = 0U;

    CHECK_NULL(pot_dev.adc_ctx);
    CHECK_NULL(pot_dev.data);

...
typedef struct { const void *pwm_ctx; uint8_t channel; void *data; } Led_Device_t;

static Led_Device_t led_dev;

static status_t light_init_wrapper(void)
{
	status_t ret  = interfaces_get("pwm_led", &led_dev.pwm_ctx, &led_dev.data);
    led_dev.channel  = 1U;

    CHECK_NULL(led_dev.pwm_ctx);
    CHECK_NULL(led_dev.data);

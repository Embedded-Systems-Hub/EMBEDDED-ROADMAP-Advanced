...
static status_t pwm_init_wrapper(void *ctx, uint8_t channel, void *data)
{
    ...
    */
    ...
    HAL_TIM_PWM_Start_DMA(htim4, tim_channel_lookup[channel], (uint32_t *)data, 1U);
...
static status_t pwm_set_wrapper(void *ctx, const void *in)
{
    ...
    CHECK_NULL(ctx);
    CHECK_RANGE(value, 0, 65535); //TIM4 resolution
